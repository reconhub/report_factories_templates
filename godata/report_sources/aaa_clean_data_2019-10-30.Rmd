...
3. relationships between cases data

4. relationships between contacts data
...
## step 3
cases_relationships_file
cases_relationships <- custom_import(cases_relationships_file) %>%
  as_tibble()


## step 4
contacts_relationships_file
contacts_relationships <- custom_import(contacts_relationships_file) %>%
  as_tibble()
...
cases_relationships <- clean_data(cases_relationships, guess_dates = FALSE)
contacts_relationships <- clean_data(contacts_relationships, guess_dates = FALSE)
...
cases_rds_file_name <- sprintf("%sclean_%s.rds",
...
destination <- here("data",
                    ...
                    cases_rds_file_name)
# Copy the rds data
file.copy(from = file.path("produced_rds", cases_rds_file_name),
          to = destination,
          overwrite = TRUE)
...
* first version
* reads in: *cases*, *contacts*, *cases_relationships*, *contacts_relationships*
* standardise data (lower case, no accents, clean separators etc.)
* convert dates used in other reports
* adds variables (epiweek, top zones)
* export stuff and create `current_clean_data.R` script
