...
Given Komanda special dataset organization, additional data management was done.

```{r data_cleaning}

x <- x_raw %>%
  mutate(origin_not_cleaned = origin) %>%
  as_tibble()
...
x <- x %>%
  mutate(fievre= ifelse(grepl(c("fiev|fier"), symptomes, ignore.case=F), "oui"
                          , "non"),
          nausee_vom= ifelse(grepl(c("naus|vertig|vom"), symptomes, ignore.case=F), "oui"
                            , "non"),
         diarrhee=ifelse(grepl(c("diar"), symptomes, ignore.case=F), "oui"
                         , "non"),
         fatigue=ifelse(grepl(c("fatig|ast"), symptomes, ignore.case=F), "oui"
                        , "non"),
         anorexie=ifelse(grepl(c("anorex"), symptomes, ignore.case=F), "oui"
                         , "non"),
         dlr_abdo=ifelse(grepl(c("abd"), symptomes, ignore.case=F), "oui"
                         , "non"),
         dlr_thorax= ifelse(grepl(c("thora|resp"), symptomes, ignore.case=F), "oui"
                            , "non"),
         dlr_muscu=ifelse(grepl(c("muscu"), symptomes, ignore.case=F), "oui"
                          , "non"),
         dlr_artic=ifelse(grepl(c("artic"), symptomes, ignore.case=F), "oui"
                          , "non"),
         cephalee=ifelse(grepl(c("cephal"), symptomes, ignore.case=F), "oui"
                         , "non"),
         toux=ifelse(grepl(c("toux"), symptomes, ignore.case=F), "oui"
                     , "non"),
         mal_de_gorge=ifelse(grepl(c("gorge"), symptomes, ignore.case=F), "oui"
                     , "non"),
         dyspnee=ifelse(grepl(c("dyspn"), symptomes, ignore.case=F), "oui"
                        , "non"),
         dysphagie=ifelse(grepl(c("dyspha"), symptomes, ignore.case=F), "oui"
                          , "non"),
         hoquet=ifelse(grepl(c("hoquet"), symptomes, ignore.case=F), "oui"
                       , "non"),
         saignements= ifelse(grepl(c("saign|sang|hemor"), symptomes, ignore.case=F), "oui"
                             , "non")
 )

#Now modify the type or origin var with lot of "autre" by using info in type and origin
##When there is PS or CS or UTI, HGR, POE: it is surv passive ie healthcare services
##telephone is recherche active

#first cretate a cat var that says whether or not a healthcare place was implicated
x <- x %>%
  mutate(hc_place= ifelse(
                    grepl(c("ps|cs|poe|hgr|uti|clini|infir"), origin_not_cleaned, ignore.case=F), "oui", "non")
  )
#|surv

x <- x %>%
  mutate(origin_final= ifelse(
    type %in% c(NA,"autre") & (hc_place=="oui" | origin_not_cleaned=="surveillance"), "surveillance_passive", type)
  ) %>%
  select(-origin)

#after this cleaning the remaining origin_final == autre seem to be "communautaire"

x <- x %>%
  mutate(origin_final= ifelse(
    origin_final=="autre", "communautaire", origin_final)
  )
...
## origin / source_notif
x <- mutate(x, origin = origin_final)
table(x$origin, useNA = "ifany")

# create variable for alert validation
x <- x %>% mutate(
  status = ifelse(ccs == "inconnu", "statut_inconnu", ccs)) %>%
