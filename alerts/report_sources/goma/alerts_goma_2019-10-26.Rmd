...
We load in population data for each Aire/Zone de Sante. 

```{r load_population_data}

# ## load the data
# pop_data <- rio::import(path_to_geog_pop)
# glimpse(pop_data)
# 
# path_to_geog_pop
...
# REVIEW: Original encoding caused an error in clean_data function (on U.S. mac) 
# pop_data <- pop_data %>% mutate_if(is.character, enc2utf8)
# pop_data <- pop_data %>%
#   clean_data(guess_dates = FALSE,
#              wordlists = cleaning_rules) %>%
#   as_tibble()
...
## origin / source_notif
x <- mutate(x, origin = origine_alerte)
...
- `as_population`, `hz_population` and `total_population` - which describe the 
population of the Aire de Sante, Zone de Sante and Sous-Coordination respectively.
...
## top zones and top aire de santes
# belong_to_goma <- pop_data$sous_coordination == "goma"
# goma_ha <- pop_data$aire_de_sante[belong_to_goma]
# goma_hz <- unique(pop_data$zone_de_sante[belong_to_goma])
# aires_to_keep <- x$zone_de_sante %in% goma_hz & x$aire_de_sante %in% goma_ha
...
# 
# pop_goma_areas <- pop_data$population[belong_to_goma]
# total_population <- sum(unique(pop_goma_areas), na.rm = TRUE)
# 
# ## adding populations to main data
# x <- x %>%
#   left_join(aire_de_sante_pop, by = c("aire_de_sante", "zone_de_sante")) %>%
#   left_join(zone_de_sante_pop, by = c("zone_de_sante")) %>% 
...
# goma_aires <- x$aire_de_sante %in% goma_ha
# number_individuals_aire_linked <- length(x$aire_de_sante[goma_aires])
...
# goma_ha # master list of aire de santes in Mambasa
# 
# # Identifying Aire de Santes with missing population data
# goma_ha_pop <- pop_data$population[belong_to_goma]
# index <- which(is.na(goma_ha_pop))
# non_pop_data <- goma_ha[index]
# 
# # Identifying number of alerts linked to Aire de Sante with 
# # missing population data
# alerts_no_pop <- sum(x$aire_de_sante == non_pop_data)
...
x_validations <- x %>%
  # filter(zone_de_sante %in% goma_hz) %>%
  # filter(aire_de_sante %in% goma_ha) %>%
...
x_validations %>% 
ggplot(aes(x = epiweek_report_label, y = count, fill = status)) +
  geom_col() +
...
ggplot(x_decisions_recent, aes(x = top_zones, y = count, 
