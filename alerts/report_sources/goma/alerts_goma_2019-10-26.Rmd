...
**Maintainer:** Your Name (your_email@something.com)

**Code contributors:** Thibaut Jombart, Chris Jarvis

**Data contributors:** Name of contributors
...
most_recent_unknown_as <- max(unknown_as$date, na.rm = T)

start_date <- database_date - 21
unknown_as_3weeks <- x %>% 
  filter(!aire_de_sante %in% as_names & date >= start_date)
...
## load the data
pop_data <- pop_data %>% 
  filter(aire_de_sante %in% x$aire_de_sante & zone_de_sante %in% x$zone_de_sante)
...
table(x$aire_de_sante[!number_individuals]) # aire de santes in database we're not matching
...
For the Goma Alerts Database, **`r round(100 * prop_linked, 2)`%** of individuals 
(`r number_individuals_aire_linked` / `r number_individuals`) had a legitimate Aire 
de Sante belonging to Goma inputted - the remainder had Aire de Santes which either
do not belong to Goma or are not an Aire de Sante. **`date_na`** alerts were removed
for an invalid date.
<!-- REVIEW: invalid dates removed ^ -->

**Note:** Population data is unavailable for the aire de santes of 
**`r non_pop_data`** which are not present in the population database. These
Aire de Santes make up **`r alerts_no_pop`** alerts. 

**Note:** These analyses **CANNOT** detect legitimate Aire de Santes missing 
from the geographical/population database. If any of these are identified as
missing, they must be manually added to the database so that they can be
incorporated into the analyses presented here. 
...
**Note:** Population data is unavailable for the Aire de Santes of 
**`r non_pop_data`** which are not present in the population database. These
Aire de Santes make up **`r alerts_no_pop`** alerts, which are included in
the calculations for Sous-Coordination and Zone de Sante. It might be worth 
excluding alerts belonging to these Aire de Santes when it comes to these
calculations. Underestimatd otherwise. **TO CHECK**

**Note:** Similarly, the "Other" group for `top_zones` and `top_aires` is not
presented in this section as this is an amalgamation of different places, some 
of which lack reliable (or even any) population data. 
...
x_alert_rate_zone_sante <- x %>%
  group_by(status, zone_de_sante, zs_population) %>%
  filter(top_zones != "other" & zs_population > 0) %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / (zs_population * number_weeks))
...
x_alert_rate_zone_sante <- x %>%
  group_by(epiweek_report_label, status, top_zones, zs_population) %>%
  filter(top_zones != "other") %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / zs_population)
...
x_alert_rate_zone_sante <- x %>%
  filter(date > database_date - 21) %>%
  group_by(status, top_zones, zs_population) %>%
  filter(as_population > 0) %>%
  filter(top_zones != "other") %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / (zs_population * 3))
...
x_alert_rate_zone_sante <- x %>%
  filter(date > database_date - 21) %>%
  group_by(date, status, top_zones, zs_population) %>%
  filter(zs_population > 0) %>%
  filter(top_zones != "other") %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / zs_population)
...
x_alert_rate_zone_sante <- x %>%
  group_by(status, top_zones, aire_de_sante, as_population) %>%
  filter(top_zones != "other") %>%
  filter(as_population > 0) %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / (as_population * number_weeks))
...
x_alert_rate_aire_sante <- x %>%
  group_by(epiweek_report_label, status, top_aires, as_population) %>%
  filter(top_aires != "other") %>%
  filter(as_population > 0) %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / as_population) 
...
x_alert_rate_aire_sante_recent <- x %>%
  ...
  filter(date > database_date - 21) %>%
  filter(as_population > 0) %>%
  group_by(status, aire_de_sante, top_zones, as_population) %>%
  summarise(count = n()) %>%
  mutate(alert_rate = (10000 * count) / (as_population * 3)) 
...
x_alert_rate_aire_sante <- x %>%
  ...
  filter(top_aires != "other") %>%
  filter(as_population > 0) %>%
  group_by(date, status, top_aires, as_population) %>%
  summarise(count = n()) %>%
  mutate(alert_rate = 10000 * count / (as_population)) 
...
```{r rds_links, results = "asis", eval = TRUE}
