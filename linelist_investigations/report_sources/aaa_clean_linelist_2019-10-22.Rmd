...
x <- x_raw %>%
  ...
  mutate(date_hosp1_start = hosp1_start) %>%
  mutate(date_hosp2_start = hosp2_start) %>%
  mutate(date_hosp3_start = hosp3_start) %>%
  mutate(date_hosp4_start = hosp4_start) %>%
  mutate(date_hosp5_start = hosp5_start) %>%
  # mutate(date_hosp6_start = hosp6_start) %>%
  mutate(date_hosp1_end = hosp1_end) %>%
  mutate(date_hosp2_end = hosp2_end) %>%
  mutate(date_hosp3_end = hosp3_end) %>%
  mutate(date_hosp4_end = hosp4_end) %>%
  mutate(date_hosp5_end = hosp5_end) %>%
  # mutate(date_hosp6_end = hosp6_end) %>%
  mutate(current_status = final_outcome) %>%
  mutate(report_status = statusreport_mdc) %>% 
  mutate(outcome = final_outcome) %>% 
  mutate(zone_de_sante = zs_cnc) %>% 
  ...
  mutate(place_death_crude = case_when(
                    current_status == "died" ~ place_of_death,
                    current_status == "alive" ~ "alive",
                    current_status == "survived" ~ "alive",
                    TRUE ~ "unknown"))
...
* google spreadsheet's origin is 1899-12-30 (for MLL)
...
x <- x %>% 
  mutate_at(
      ...
      guess_dates, origin = as.Date("1899-12-30"),
      error_tolerance = 0.1)
...
x <- x %>%
  mutate(type_death =
           case_when(
               ...
               str_detect(place_death_crude, "community") ~ "community",
