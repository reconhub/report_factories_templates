...
<!-- ======================================================= -->
<!-- ======================================================= -->
# Nosocomial deaths {.tabset .tabset-fade .tabset-pills}

<!-- ======================================================= -->
## Outline 

We use the variable `place_death_crude` which is derived from the 
variable `place_of_death` to estimate the proportions of cases
who are known to have died in a health facility (which is a subset
of the community or non-CTE deaths):
...
```{r nosocomial_deaths_overall}

## step 1
nosocomial_deaths_overall <- x %>%
  count(place_death_crude) %>%
  spread(place_death_crude, n, fill = 0, drop = FALSE) %>%
  mutate(total = nrow(x), # avoid adorn_totals
         denom = total - unknown,
         p = prop_to_perc(community_hcf / denom),
         p_lower = prop_ci(community_hcf, denom, "lower", perc = TRUE),
         p_upper = prop_ci(community_hcf, denom, "upper", perc = TRUE))
...
## step 3
ggplot(nosocomial_deaths_overall_time, aes(x = epiweek_report_label, y = p)) +
  theme_bw() +
  geom_ribbon(aes(ymin = p_lower, ymax = p_upper),
              alpha = .3, fill = color_nosocomial_death) +
  geom_point(color = color_nosocomial_death) +
  geom_line(color = color_nosocomial_death) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_months +
  rotate_x_text(45) +
  large_txt +
  labs(title = "Proportion de décès nosocomial",
       x = "Semaine de notification",
       y = "% de cas hebdomadaires décès nosocomial")


## step 4
table_nosocomial_deaths_overall <- nosocomial_deaths_overall %>% 
  select("% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
         "IC95% upper" = p_upper,
         n = denom,
         everything())

table_nosocomial_deaths_overall %>%
  show_table()


# step 5
table_nosocomial_deaths_overall_time <- nosocomial_deaths_overall_time %>% 
  select(semaine = epiweek_report_label,
         "% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
         "IC95% upper" = p_upper,
         n = denom,
         everything())

table_nosocomial_deaths_overall_time %>%
  show_table()
...
```{r nosocomial_deaths_sub_coordination}
...
## step 2
nosocomial_deaths_sub_coordination_time <- x %>%
  count(sous_coordination, month_report, place_death_crude) %>%
  spread(place_death_crude, n, fill = 0) %>%
  adorn_totals(c("col"), name = "total") %>% 
  mutate(denom = total,
         p = prop_to_perc(community_hcf / denom),
         p_lower = prop_ci(community_hcf, denom, "lower", perc = TRUE),
...
## step 3
ggplot(nosocomial_deaths_sub_coordination, aes(x = sous_coordination, y = p)) +
  theme_bw() +
  geom_errorbar(aes(ymin = p_lower, ymax = p_upper),
                color = color_nosocomial_death) +
  geom_point(color = color_nosocomial_death, size = 2) +
  geom_label(aes(y = p_upper + 10, label = paste0("n=", denom))) +
  large_txt +
  coord_flip() +
  scale_y_continuous(breaks = seq(0, 100, by = 20),
  ...
  labs(title = "Proportion de décès nosocomial\npar sous-coordination",
       x = "",
       y = "% de décès nosocomial")


## step 4
to_keep <- nosocomial_deaths_sub_coordination_time %>%
  group_by(sous_coordination) %>%
  filter(mean(p_upper - p_lower) < 60) %>%
  pull(sous_coordination) %>%
  unique()

nosocomial_deaths_sub_coordination_time %>%
  filter(sous_coordination %in% to_keep) %>% 
ggplot(aes(x = month_report, y = p)) +
  theme_bw() +
  geom_ribbon(aes(ymin = p_lower, ymax = p_upper),
...
## step 5
table_nosocomial_deaths_sub_coordination <- nosocomial_deaths_sub_coordination %>% 
  select(sous_coordination,
         "% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
         "IC95% upper" = p_upper,
...
table_nosocomial_deaths_sub_coordination %>%
  show_table()


# step 6
table_nosocomial_deaths_sub_coordination_time <- nosocomial_deaths_sub_coordination_time %>% 
  select(mois = month_report,
         sous_coordination,
         "% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
...
table_nosocomial_deaths_sub_coordination_time %>%
  show_table()
...
```{r nosocomial_deaths_top_zones}

## step 1
nosocomial_deaths_top_zones <- x %>%
...
## step 2
nosocomial_deaths_top_zones_time <- x %>%
  count(top_zones, month_report, place_death_crude) %>%
  spread(place_death_crude, n, fill = 0) %>%
  adorn_totals(c("col"), name = "total") %>% 
  mutate(denom = total,
         p = prop_to_perc(community_hcf / denom),
...
## step 3
ggplot(nosocomial_deaths_top_zones, aes(x = top_zones, y = p)) +
  theme_bw() +
  geom_errorbar(aes(ymin = p_lower, ymax = p_upper),
                color = color_nosocomial_death) +
  geom_point(color = color_nosocomial_death, size = 2) +
...
## step 4
to_keep <- nosocomial_deaths_top_zones_time %>%
  group_by(top_zones) %>%
  filter(mean(p_upper - p_lower) < 60) %>%
  pull(top_zones) %>%
  unique()

nosocomial_deaths_top_zones_time %>%
  filter(top_zones %in% to_keep) %>% 
ggplot(aes(x = month_report, y = p)) +
  theme_bw() +
  geom_ribbon(aes(ymin = p_lower, ymax = p_upper),
...
## step 5
table_nosocomial_deaths_top_zones <- nosocomial_deaths_top_zones %>% 
  select(top_zones,
         "% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
         "IC95% upper" = p_upper,
...
table_nosocomial_deaths_top_zones %>%
  show_table()


# step 6
table_nosocomial_deaths_top_zones_time <- nosocomial_deaths_top_zones_time %>% 
  select(mois = month_report,
         top_zones,
         "% nosocomial_death connu" = p,
         "IC95% lower" = p_lower,
...
table_nosocomial_deaths_top_zones_time %>%
  show_table()
...
completeness <- data.frame(
    ...
    community_or_nosocomial_deaths = 100 * community_deaths_overall$denom / community_deaths_overall$total,
